...
test("ships a consistent SHNL 36-0 search and install identity", async () => {
  ...
  assert.match(page, /koju je napravio Josip\s+Nigojević/);
...
test("ships cookie-authenticated accounts with durable season history", async () => {
  ...
  assert.match(page, /"\/account\/password-reset\/request"/);
  assert.match(page, /"\/account\/password-reset\/complete"/);
  ...
  assert.match(page, /type AccountDialogMode = "login" \| "register" \| "forgot" \| "reset"/);
  assert.match(page, /Zaboravili ste lozinku\?/);
  assert.match(page, /Nastavi promjenu lozinke/);
  assert.match(page, /PASSWORD_RESET_FRAGMENT = "#reset-password="/);
  assert.match(page, /passwordResetTokenFromFragment\(window\.location\.hash\)/);
  assert.match(page, /cleanUrl\.hash = ""/);
  assert.match(page, /newPassword: accountForm\.password/);
  assert.match(page, /accountForm\.password !== accountForm\.passwordConfirmation/);
  assert.match(page, /minLength=\{settingNewPassword \? 15 : undefined\}/);
  assert.match(page, /minLength=\{15\}/);
  assert.match(page, /Račun nije obavezan za igru/);
  assert.match(css, /\.account-modal-backdrop/);
  assert.match(css, /\.account-forgot/);
  assert.match(css, /\.account-recovery-actions/);
  assert.match(css, /min-height:\s*44px/);
  assert.match(css, /\.account-form-notice/);
